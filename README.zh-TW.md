...
- **由使用者決定是否改善：**Xylon 顯示有期限的確切提案。使用者必須在本機頁面
  輸入提案代碼，Xylon 才能執行一次 candidate，並用相同指標比較前後結果。
...
5. 查看實測 WNS／TNS 與確切提案；只有確定要執行一次 candidate 時才輸入代碼。

高負載 EDA 工作一次只執行一項，並先檢查本機資源。預設使用 1 CPU、8 GiB
記憶體上限、容器禁止連網，且只清理由 Xylon 建立的資源。容量低於安全門檻時，
Xylon 不會啟動 OpenROAD。

...
test('produces stable SHA-256 identities and changes design identity with exact input', () => {
  ...
  assert.equal(first.report_recipe.flow.stage, 'grt')
  assert.equal(first.report_recipe.flow.platform, 'sky130hd')
  assert.match(first.report_recipe.flow.image, /^openroad\/orfs@sha256:[a-f0-9]{64}$/)
  assert.equal(first.report_recipe.flow.skipCtsRepairTiming, true)

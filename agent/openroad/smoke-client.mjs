...
const client = new Client({ name: 'xylon-openroad-smoke', version: '0.1.0' })
...
try {
  ...
  for (const required of [
    ...
    'execute_approved_openroad_change',
    'get_openroad_metrics',
    'terminate_openroad_session',
  ]) assert.ok(names.has(required), `missing MCP tool ${required}`)
  const destructiveTool = tools.tools.find((tool) => tool.name === 'execute_approved_openroad_change')
  ...
  const mismatched = await client.callTool({
    name: 'execute_approved_openroad_change',
    arguments: {
      approval_id: preparedPayload.approval_id,
  ...
  assert.equal(mismatchedPayload.error, 'ApprovalCommandMismatch')
  ...
  const executed = await client.callTool({
    name: 'execute_approved_openroad_change',
    arguments: {
      approval_id: preparedAgainPayload.approval_id,
  ...
  process.stdout.write(`${JSON.stringify({
    ...
    approved_change: executedPayload.output.trim(),
  })}\n`)
} finally {
  if (sessionId) {
